...
public class CashierView extends BorderPane {
    ...
    private void buildLayout() {
        ...
        orderPanel.setStyle("-fx-background-color: white; -fx-border-color: #ddd; -fx-border-radius: 6; -fx-background-radius: 6;");
    ...
    private static class OrderLine {
        ...
        OrderLine(MenuItem menuItem, int quantity, int index) {
    ...
    private void addToOrder(MenuItem item) {
        ...
        currentOrder.add(new OrderLine(item, 1, currentOrder.size()));
        refreshOrderDisplay();
    }
    ...
    private void refreshOrderDisplay() {
        ...
        for (int i = 0; i < currentOrder.size(); i++) {
            OrderLine line = currentOrder.get(i);
            HBox row = new HBox(8);
            row.setAlignment(Pos.CENTER_LEFT);
            Label lbl = new Label(line.quantity + "x " + line.menuItem.getName() + "  $" + line.menuItem.getBasePrice().multiply(BigDecimal.valueOf(line.quantity)).setScale(2, RoundingMode.HALF_UP));
        ...
        for (OrderLine line : currentOrder) {
            subtotal = subtotal.add(line.menuItem.getBasePrice().multiply(BigDecimal.valueOf(line.quantity)));
        }
        BigDecimal taxTip = subtotal.multiply(TAX_RATE).add(subtotal.multiply(TIP_RATE)).setScale(2, RoundingMode.HALF_UP);
    ...
    private void editQuantity(OrderLine line) {
        ...
        d.showAndWait().ifPresent(s -> {
            try {
                ...
                if (q <= 0) currentOrder.remove(line);
    ...
    private void submitOrder() {
        if (currentOrder.isEmpty()) {
            new Alert(Alert.AlertType.WARNING, "Add at least one item to the order.").showAndWait();
            return;
        }
        ...
        SalesOrder order = new SalesOrder(orderId, new Timestamp(System.currentTimeMillis()), total, "Cash");
        for (OrderLine line : currentOrder) {
            SalesOrderItem oi = new SalesOrderItem(nextOrderItemId++, orderId, line.menuItem.getMenuItemId(), line.quantity, line.menuItem.getBasePrice());
        ...
        if (result.contains("success")) {
            currentOrder.clear();
            refreshOrderDisplay();
            new Alert(Alert.AlertType.INFORMATION, "Order submitted. Total: $" + total).showAndWait();
        } else {
