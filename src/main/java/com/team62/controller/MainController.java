/* @author
* Team 62: Rishii Vijay, Adam Ryu, Ryan Nguyen, Grant Gallun, Brice Ockman, Matthew Culver
*/
...
public class MainController {
   ...
   /**
   * @return list of all menu items; dbId is set to item_id for update/delete operations.
   */
   ...
   */
   ...
   /**
   * @param item the menu item to delete; must have dbId set to attempt delete
   ...
   */
   ...
   /**
   * @return list of all inventory items, with name and quantity; dbId is set to inventory_id for update/delete operations.
   */
   ...
   */
   ...
   /**
   * @param item the inventory item to delete; must have dbId set to attempt delete
   ...
   */
   ...
   /**
   * @return list of all employees; role and active status are parsed from the work_history JSONB field.
   * If work_history is missing or malformed, defaults to role="" and active=true.
   * On query error, returns an empty list and prints the stack trace.
   */
   ...
   * @param employee the employee to delete; must have dbId set to attempt delete
   */
   ...
   /**
   * @param date the date to calculate total sales for (time component ignored)
   * @return the total sales amount for orders placed on the given date; returns 0
   * if no sales or if an error occurs during query execution
   */
   ...
   /**
   * @param date the date to count orders for (time component ignored)
   * @return the count of orders placed on the given date; returns 0 if none or if an error occurs
   */
   ...
   /**
   * @param order the order to process; must have at least one item with valid itemDbId and quantity > 0
   * @return success or error message; on success, the order is persisted to the database and added to the in-memory list
   */
   ...
   * @return a valid employee_id from the database, creating a demo employee if needed
   */
   ...
   /**
   * @param conn an open SQL connection
   * @return a valid customer_id from the database, creating a demo customer if needed
   */
