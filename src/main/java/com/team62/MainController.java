...
public class MainController {
    ...
    public void updateInventoryItem(InventoryItem item) {
        ...
        try (var conn = Database.getConnmapection()) {
            conn.setAutoCommit(false);
            try (var ps = conn.prepareStatement("""
    ...
    public Map<String, Integer> getInventoryUsageData(LocalDate start, LocalDate end) {
    Map<String, Integer> usageData = new LinkedHashMap<>();

    String sql = """
            SELECT COALESCE(meta.display_name, 'Inventory Item') AS item_name,
                   COALESCE(SUM(u.quantity_used), 0) AS used_total
              FROM pos_inventory_usage u
         LEFT JOIN pos_inventory_meta meta
                ON meta.inventory_id = u.inventory_id
             WHERE u.business_date BETWEEN ? AND ?
          GROUP BY item_name
          ORDER BY used_total DESC, item_name
            """;
