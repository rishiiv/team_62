...
public class ManagerView extends BorderPane {
    ...
    usageChart.setTitle("Product Usage");
    usageChart.setLegendVisible(false);
    usageChart.setAnimated(false);
    usageChart.setCategoryGap(12);
    usageChart.setBarGap(4);

    // Stack the chart and text output in same area
    StackPane displayPane = new StackPane(output, usageChart);
    output.toFront();
    VBox.setVgrow(displayPane, Priority.ALWAYS);

    Button productUsageBtn = new Button("Product Usage Chart");
    productUsageBtn.setOnAction(e -> {
        usageChart.getData().clear();

        Map<String, Integer> usageData =
                controller.getInventoryUsageData(startPicker.getValue(), endPicker.getValue());
        ...
        usageChart.setTitle("Product Usage: " + startPicker.getValue() + " to " + endPicker.getValue());
        usageChart.toFront();
    });
    ...
    zReportBtn.setOnAction(e -> {
        output.setText(controller.runZReport(LocalDate.now()));
        output.toFront();
    });

    Button salesReportBtn = new Button("Sales Report");
    salesReportBtn.setOnAction(e -> {
        output.setText(controller.getSalesReport(startPicker.getValue(), endPicker.getValue()));
        output.toFront();
    });

    Button restockBtn = new Button("Restock Report");
    restockBtn.setOnAction(e -> {
        output.setText(controller.getRestockReport());
        output.toFront();
    });
    ...
    reportsPane.getChildren().addAll(title, new Separator(), top, displayPane);
